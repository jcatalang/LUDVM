...
class LUDVM():
    ...
    def induced_velocity(self, circulation, xw, zw, xp, zp, viscous = True):
          ...
          # vortices located at 'xw,yw', using the Vatista's vortex model
          # with core-radius v_core = 1.3*dt_star*chord
          Np, Nw = len(xp), len(xw)
    ...
    def compute_coefficients(self):
        ...
        q = 0.5*self.rho*self.Uinf**2*self.chord

        self.Cp = self.dp/q
        self.Cn, self.Cs = self.Fn/q, self.Fs/q
        self.Cl, self.Cd = self.L/q , self.D/q
        self.Cm = self.M/(q*self.chord)
